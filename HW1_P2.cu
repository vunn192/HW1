		...
		uchar3 * outPixels)
{
	...
	{ 	
		...
		for (int k = 0; k < filterWidth; k++){
			int inPixels_x = r - d + k;
			if (inPixels_x < 0)
				inPixels_x = 0;
			
			if (inPixels_x > height - 1)
				inPixels_x = height - 1;

			for (int l = 0; l < filterWidth; l++){
				int filter_index = k * filterWidth + l;
				int inPixels_y = c - d + l;
				if (inPixels_y < 0)
					inPixels_y = 0;
			
				if (inPixels_y > width - 1)
					inPixels_y = width - 1;
								
				tmp_x += filter[filter_index] * inPixels[inPixels_x * width + inPixels_y].x;
				tmp_y += filter[filter_index] * inPixels[inPixels_x * width + inPixels_y].y;
				tmp_z += filter[filter_index] * inPixels[inPixels_x * width + inPixels_y].z;
			}
		}
		outPixels[i].x = tmp_x;
		outPixels[i].y = tmp_y;
		...
		bool useDevice=false, dim3 blockSize=dim3(1, 1))
{
	...
	if (useDevice == false)
	{
		...
		for (int r = 0; r < height; r++){
			for (int c = 0; c < width; c++){
				...
				int i = r * width + c;
				for (int k = 0; k < filterWidth; k++){
					int inPixels_y = r - d + k;
					if (inPixels_y < 0)
						inPixels_y = 0;
					
					if (inPixels_y > height - 1)
						inPixels_y = height - 1;

					for (int l = 0; l < filterWidth; l++){
						int filter_index = k * filterWidth + l;
						int inPixels_x = c - d + l;
						if (inPixels_x < 0)
							inPixels_x = 0;
						
						if (inPixels_x > width - 1)
							inPixels_x = width - 1;
						
						tmp_x += filter[filter_index] * inPixels[inPixels_y * width + inPixels_x].x;
						tmp_y += filter[filter_index] * inPixels[inPixels_y * width + inPixels_x].y;
						tmp_z += filter[filter_index] * inPixels[inPixels_y * width + inPixels_x].z;
					}
				}
				outPixels[i].x = tmp_x;
				outPixels[i].y = tmp_y;
	...
	else // Use device
	{
		...
		dim3 gridSize((height - 1) / blockSize.x + 1, 
				(width - 1) / blockSize.y + 1);
